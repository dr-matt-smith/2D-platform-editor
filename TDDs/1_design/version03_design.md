...
- `tileset.js` atlas path swapped to the new location; a
  `public/data/levels/manifest.json` generated by a `predev`/`prebuild` hook
  so the app can enumerate levels and the manifest can't go stale.
...
`public/data/levels/*.txt`, parse each `# name:`, write `manifest.json`. It is
wired into `package.json` as a `predev`/`prebuild` hook so it regenerates
before every `npm run dev` / `npm run build` and cannot go stale; it can also
be run on demand (documented in `data/levels/README.md`).
...
| # | Deliverable |
|---|-------------|
| 1 | Move data to `public/data/`; `tileset.js` path swap; manifest generator script wired as `predev`/`prebuild` hook; build/tests green; slicing + doc paths updated |
...
assets + loading; reachability lint if not pulled into v3.
